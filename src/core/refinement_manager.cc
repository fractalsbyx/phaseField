...
RefinementManager<dim, degree, number>::mark_cells_for_refinement_and_coarsening()
{
  ...
  const auto gradient_criterion =
    [](const RefinementCriterion                        &criterion,
       ...
       std::vector<std::vector<Tensor<1, dim, number>>> &vector_gradients) -> bool
    {
      if (rank == TensorRank::Scalar)
        {
      ...
      // Check if any of the quadrature points meet the refinement criterion
      return std::any_of(vector_gradients.begin(),
                         vector_gradients.end(),
                         [&](const std::vector<Tensor<1, dim, number>> &vector_gradient)
                           {
                             // @todo clean up this allocation
                             dealii::Vector<number> vector_gradient_component_magnitude(
                               dim);
                             for (unsigned int dimension = 0; dimension < dim;
                                  dimension++)
                               {
  ...
  std::vector<Tensor<1, dim, number>>              gradients(num_quad_points);
  std::vector<std::vector<Tensor<1, dim, number>>> vector_gradients(
    num_quad_points,
    std::vector<Tensor<1, dim, number>>(dim));
