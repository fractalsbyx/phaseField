...
variableContainer<dim, degree, T>::variableContainer(
  ...
  : src_attributes(std::move(_src_attributes))
  , dst_attributes(std::move(_dst_attributes))
  , solve_type(_solve_type)
{
  for (const auto &[var_index, src_variable] : src_attributes)
    {
      uint      dof_no           = var_index;
      EvalFlags evaluation_flags = src_variable.eval_flags_for_solve(solve_type);
      bool      var_needed       = bool(evaluation_flags) || src_variable.is_pp;
      bool      change_needed    = bool(src_variable.eval_flags_change_nonexplicit_LHS);
      if (var_needed)
        {
          if (src_variable.var_type == SCALAR)
            {
              src_scalar_vars_map.emplace(var_index, data, dof_no);
            }
          else
            {
              src_vector_vars_map.emplace(var_index, data, dof_no);
            }
        }
      if (change_needed)
        {
          if (src_variable.var_type == SCALAR)
            {
              scalar_change_in_vars_map.emplace(var_index, data, dof_no);
            }
          else
            {
              vector_change_in_vars_map.emplace(var_index, data, dof_no);
            }
        }
    }
  for (const auto &[var_index, dst_variable] : dst_attributes)
    {
      uint dof_no = (solve_type != solveType::POSTPROCESS) ? var_index : 0;
      if (dst_variable.var_type == SCALAR)
        {
          dst_scalar_vars_map.emplace(var_index, data, dof_no);
        }
      else
        {
          dst_vector_vars_map.emplace(var_index, data, dof_no);
        }
    }
}
...
{
  if (!src_scalar_vars_map.empty())
    {
      return src_scalar_vars_map.begin()->second->n_q_points;
    }
  else if (!src_vector_vars_map.empty())
    {
      return src_vector_vars_map.begin()->second->n_q_points;
    }
...
variableContainer<dim, degree, T>::get_q_point_location() const
{
  if (!src_scalar_vars_map.empty())
    {
      return src_scalar_vars_map.begin()->second->quadrature_point(q_point);
    }
  else if (!src_vector_vars_map.empty())
    {
      return src_vector_vars_map.begin()->second->quadrature_point(q_point);
    }
  else if (!scalar_change_in_vars_map.empty())
    {
                                                   ...
                                                   unsigned int                     cell)
{
  for (const auto &[var_index, src_variable] : src_attributes)
    {
      if (src_variable.var_needed(solve_type) || src_variable.is_pp)
        {
          const uint dof_no = var_index;
          if (src_variable.var_type == SCALAR)
            {
              auto &var_FEEval = src_scalar_vars_map.at(var_index);
              var_FEEval.reinit(cell);
              var_FEEval.read_dof_values(*src[dof_no]);
              var_FEEval.evaluate(src_variable.eval_flags_for_solve(solve_type));
            }
          else
            {
              auto &var_FEEval = src_vector_vars_map.at(var_index);
              var_FEEval.reinit(cell);
              var_FEEval.read_dof_values(*src[dof_no]);
              var_FEEval.evaluate(src_variable.eval_flags_for_solve(solve_type));
            }
...
{
  const variableAttributes src_variable = src_attributes.at(var_index);
  if (src_variable.var_type == SCALAR)
    {
      auto &var_FEEval = scalar_change_in_vars_map.at(var_index);
      var_FEEval.reinit(cell);
      var_FEEval.read_dof_values(src);
      var_FEEval.evaluate(src_variable.eval_flags_residual_nonexplicit_LHS);
    }
  else
    {
      auto &var_FEEval = vector_change_in_vars_map.at(var_index);
      var_FEEval.reinit(cell);
      var_FEEval.read_dof_values(src);
      var_FEEval.evaluate(src_variable.eval_flags_residual_nonexplicit_LHS);
    }
...
template <int dim, int degree, typename T>
void
variableContainer<dim, degree, T>::reinit_dst(unsigned int cell)
{
  for (const auto &[var_index, dst_variable] : dst_attributes)
    {
      if (dst_variable.var_type == SCALAR)
        {
          dst_scalar_vars_map.at(var_index).reinit(cell);
        }
      else
        {
          dst_vector_vars_map.at(var_index).reinit(cell);
        }
    }
}
  ...
  for (const auto &[var_index, dst_variable] : dst_attributes)
    {
      ...
      if (dst_variable.var_type == SCALAR)
        {
          auto &var_FEEval = dst_scalar_vars_map.at(var_index);
          var_FEEval.integrate(dst_variable.residual_flags_for_solve(solve_type));
          var_FEEval.distribute_local_to_global(*dst[dof_no]);
        }
      else
        {
          auto &var_FEEval = dst_vector_vars_map.at(var_index);
          var_FEEval.integrate(dst_variable.residual_flags_for_solve(solve_type));
          var_FEEval.distribute_local_to_global(*dst[dof_no]);
        }
    }
...
{
  // integrate
  const variableAttributes dst_variable = dst_attributes.at(var_index);
  if (dst_variable.var_type == SCALAR)
    {
      auto &var_FEEval = scalar_change_in_vars_map.at(var_index);
      var_FEEval.integrate(dst_variable.residual_flags_for_solve(solve_type));
      var_FEEval.distribute_local_to_global(dst);
    }
  else
    {
      auto &var_FEEval = vector_change_in_vars_map.at(var_index);
...
template <int dim, int degree, typename T>
void
variableContainer<dim, degree, T>::AssertValid(
  [[maybe_unused]] const uint      &var_index,
  [[maybe_unused]] const fieldType &field_type,
  [[maybe_unused]] const EvalFlags &eval_flag,
  [[maybe_unused]] bool             is_change) const
{
#ifdef DEBUG
  std::string field_string = (field_type == SCALAR) ? "scalar" : "vector";
  Assert((src_attributes.find(var_index) != src_attributes.end()) &&
           bool((!is_change
                   ? src_attributes.at(var_index).eval_flags_for_solve(solve_type)
                   : src_attributes.at(var_index).eval_flags_change_nonexplicit_LHS) &
                eval_flag),
         dealii::ExcMessage(
  ...
  Assert(src_attributes.at(var_index).var_type == field_type,
         dealii::ExcMessage(
  ...
  unsigned int global_variable_index) const
{
  ...
  return src_scalar_vars_map.at(global_variable_index).get_value(q_point);
}
  ...
  unsigned int global_variable_index) const
{
  ...
  return src_scalar_vars_map.at(global_variable_index).get_gradient(q_point);
}
  ...
  unsigned int global_variable_index) const
{
  ...
  return src_scalar_vars_map.at(global_variable_index).get_hessian(q_point);
}
  ...
  unsigned int global_variable_index) const
{
  ...
  return src_vector_vars_map.at(global_variable_index).get_value(q_point);
}
  ...
  unsigned int global_variable_index) const
{
  ...
  return src_vector_vars_map.at(global_variable_index).get_gradient(q_point);
}
  ...
  unsigned int global_variable_index) const
{
  ...
  return src_vector_vars_map.at(global_variable_index).get_hessian(q_point);
}
...
variableContainer<dim, degree, T>::set_scalar_value_term_RHS(
  ...
  T            val)
{
  dst_scalar_vars_map[global_variable_index].submit_value(val, q_point);
}
...
{
  dst_scalar_vars_map[global_variable_index].submit_gradient(grad, q_point);
}
...
{
  dst_vector_vars_map[global_variable_index].submit_value(val, q_point);
}
...
{
  dst_vector_vars_map[global_variable_index].submit_gradient(grad, q_point);
}
