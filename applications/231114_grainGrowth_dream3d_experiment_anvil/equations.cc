...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    for (unsigned int var_index=0; var_index<20; var_index++){
    ...
    //for (unsigned int var_index=0; var_index<12; var_index++){
        ...
        // For the input file 'parameters.in'
        set_dependencies_value_term_RHS(var_index, "n0, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13, n14, n15, n16, n17, n18, n19");
        set_dependencies_gradient_term_RHS(var_index, "grad(n0), grad(n1), grad(n2), grad(n3), grad(n4), grad(n5), grad(n6), grad(n7), grad(n8), grad(n9), grad(n10), grad(n11), grad(n12), grad(n13), grad(n14), grad(n15), grad(n16), grad(n17), grad(n18), grad(n19)");
...
for (unsigned int i=0; i<userInputs.number_of_variables; i++){
	...
	// Manually set the outer grain to not evolve after t=50 (in this case it's variable n1)
	if (t_n <= userInputs.dtValue*50){
		if (i == 1){
			value_terms[i] = ni-constV(userInputs.dtValue*1.0)*fnV;;
			gradient_terms[i] = constV(-userInputs.dtValue*KnV*3.0)*nix;
		}
	}

	else if (t_n > userInputs.dtValue*50){
		if (i == 1){
			value_terms[i] = ni;
			gradient_terms[i] = gradient_terms[i]*0.0;
		}
	}
}
