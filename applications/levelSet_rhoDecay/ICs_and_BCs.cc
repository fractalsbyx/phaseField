...
void customPDE<dim,degree>::setInitialCondition(const dealii::Point<dim> &p, const unsigned int index, double & scalar_IC, dealii::Vector<double> & vector_IC){
    ...
    /*
    double center[1][3] = {{0.4,0.15,0.0}};
    double dist;
    for (unsigned int i=0; i<1; i++){
        ...
        scalar_IC += std::tanh((p[0]-50)/(1.0));

    }*/
    //scalar_IC = scalar_IC/2.0;
    /*
    double center[1][3] = {{0.5,0.5,0.0}};
    double rad[1] = {5};
    double dist;
    scalar_IC = 1.0;
    ...
    for (unsigned int i=0; i<1; i++){
        ...
        scalar_IC -= (1.0-std::tanh((dist-rad[i])/1.0));
        //scalar_IC += 0.5*(1.0-std::tanh((p[0]-20)/(sqrt(2))));
    }
    */
